...
def test_validate_humanoid_path_rejects_missing_prim():
    ...
    tracker_assets = SimpleNamespace(
        robot_config=SimpleNamespace(
            kinematic_info=SimpleNamespace(body_names=("Pelvis",), joint_names=("Pelvis_tx",))
        )
    )
...
def test_controller_initialization_binds_humanoid_and_checkpoint(tmp_path):
    ...
    def load_assets(path):
        call_order.append(("load_assets", Path(path)))
        return expected_assets

    def bind_humanoid(prim_path, *, lookup_articulation, tracker_assets):
        call_order.append(("bind_humanoid", prim_path, tracker_assets))
        return expected_bound_humanoid

    controller = MotionController(
        humanoid_prim_path="/World/Humanoid",
        checkpoint_path=checkpoint,
        lookup_articulation=lambda _prim_path: object(),
        bind_humanoid=bind_humanoid,
    ...
    assert call_order == [
        ("load_assets", checkpoint),
        ("bind_humanoid", "/World/Humanoid", expected_assets),
    ]
...
def test_controller_rejects_overlapping_run_requests(tmp_path):
    ...
    controller = MotionController(
        ...
        checkpoint_path=checkpoint,
        lookup_articulation=lambda _prim_path: object(),
        bind_humanoid=lambda prim_path, *, lookup_articulation, tracker_assets: object(),
...
def test_build_runtime_reuses_motion_tracker_agent_config():
    ...
    runtime = MotionRunner(tracker_assets=assets)
...
def test_runtime_extends_episode_length_for_full_motion(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("human_motion_isaacsim.motion_runner.ensure_protomotions_importable", lambda: tmp_path)
...
def test_compile_video_uses_hymotion_codec_settings(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(viewport_capture, "ImageSequenceClip", _FakeClip)

    frame_paths = [tmp_path / "000000.png", tmp_path / "000001.png"]
    for frame_path in frame_paths:
        frame_path.write_bytes(b"png")

    viewport_capture.compile_video(frame_paths, tmp_path / "clip.mp4", fps=30)
