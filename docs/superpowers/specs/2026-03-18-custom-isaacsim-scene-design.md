...
| `_get_simulator_bodies_contact_buf(env_ids)` | Return contact force buffer; requires `ContactSensor` setup on humanoid bodies during scene creation, or return zeros as stub |
...
**Contact sensors:** Body contact reporting (`_get_simulator_bodies_contact_buf`) requires explicit `ContactSensor` setup on humanoid bodies. For the initial implementation, this will return a zero tensor (stub). If the ProtoMotions policy does not use contact observations (check `env_config`), this is safe. If it does, contact sensors must be added during scene creation.
...
2. **Contact sensor stub** — if the ProtoMotions policy uses contact observations, the zero-tensor stub will produce incorrect behavior. Mitigation: check `env_config` for contact usage; add contact sensors if needed.
